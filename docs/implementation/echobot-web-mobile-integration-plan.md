...
| 11 | Full regression | 已完成 | 本輪全量 pytest 已通過：`312 passed`，含 Open WebUI bridge、Model Profiles、Character Profiles、Stage emotion/motion event、channel secret redaction、admin allowlist、gateway user-scope 與 local model roleplay fallback 測試 | `.venv/bin/python -m pytest` | 無 |
...
| 18 | Character Profiles 角色設定頁 | 已完成第一版 | 新增 `/admin/characters` 三語角色設定頁與 `/api/character-profiles*` 聚合 API；角色 prompt、model profile 綁定、有效 chat/TTS/ASR/Live2D 摘要可在同一頁管理；資料仍沿用 role card 與 model profile binding，不新增第二套角色儲存格式 | targeted pytest、web static pytest、`node --check`、`py_compile` | 後續補角色 package 匯入/匯出 |
| 19 | Stage emotion / motion event | 已完成第一版 | `StageEvent` 與 Open WebUI bridge 可攜帶 `emotion`、`expression`、`motion`；新增 `character_state` event；`/stage` 會依 Live2D config 套用 expression/motion；`/messenger` final 回覆支援 `[emotion:...] [expression:...] [motion:...]` 標籤並只以 `textContent` 顯示文字 | `312 passed`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描 | 下一步可做角色 package 匯入/匯出 |
| 20 | Character emotion map | 已完成本機版 | `/admin/characters` 可為每個角色新增、命名與維護 emotion-to-expression/motion map；`/api/stage/events` 與 Open WebUI bridge 在事件缺少 explicit expression/motion 時，會依目前 session 綁定角色自動套用 map | `312 passed`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描 | 真機 Live2D assets 的表情/動作名稱仍需依模型檔驗證 |
| 21 | PR 更新與收尾 | 未開始 | PR #1 存在且 mergeable | `git push`、GitHub PR check | 等本機第二輪驗證完成後補 commit |
...
| Regression | 全量 pytest | 已完成，`312 passed` |
...
| 11 | Full regression | Done | This round's full pytest passed: `312 passed`, including Open WebUI bridge, Model Profiles, Character Profiles, Stage emotion/motion event, channel secret redaction, admin allowlist, gateway user-scope, and local model roleplay fallback tests | `.venv/bin/python -m pytest` | None |
...
| Regression | Full pytest | Done, `312 passed` |
