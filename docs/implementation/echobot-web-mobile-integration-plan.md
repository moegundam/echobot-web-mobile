...
| 11 | Full regression | 已完成 | 本輪全量 pytest 已通過：`310 passed`，含 Open WebUI bridge、Model Profiles、Character Profiles、channel secret redaction、admin allowlist、gateway user-scope 與 local model roleplay fallback 測試 | `.venv/bin/python -m pytest` | 無 |
...
| 18 | Character Profiles 角色設定頁 | 已完成第一版 | 新增 `/admin/characters` 三語角色設定頁與 `/api/character-profiles*` 聚合 API；角色 prompt、model profile 綁定、有效 chat/TTS/ASR/Live2D 摘要可在同一頁管理；資料仍沿用 role card 與 model profile binding，不新增第二套角色儲存格式 | targeted pytest、web static pytest、`node --check`、`py_compile` | 下一階段補 emotion/motion map 與角色 package 匯入/匯出 |
| 19 | PR 更新與收尾 | 未開始 | PR #1 存在且 mergeable | `git push`、GitHub PR check | 等本機第二輪驗證完成後補 commit |
...
| Regression | 全量 pytest | 已完成，`310 passed` |
...
| 11 | Full regression | Done | This round's full pytest passed: `310 passed`, including Open WebUI bridge, Model Profiles, Character Profiles, channel secret redaction, admin allowlist, gateway user-scope, and local model roleplay fallback tests | `.venv/bin/python -m pytest` | None |
...
| Regression | Full pytest | Done, `310 passed` |
