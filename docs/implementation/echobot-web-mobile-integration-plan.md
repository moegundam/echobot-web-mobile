...
| 11 | Full regression | 已完成 | 本輪全量 pytest 已通過：`349 passed, 2 warnings, 16 subtests passed`，含 Open WebUI bridge、Model Profiles、Character Profiles、Character Packages、Stage emotion/motion event、channel secret redaction、admin allowlist、channel smoke、gateway user-scope、Telegram/Discord true E2E support、Discord native adapter 與 local model roleplay fallback 測試 | `.venv/bin/python -m pytest -q` | 無 |
...
| 14 | Open WebUI 操作員 Bridge 介面 | 已完成接口、本機 smoke 與 GB10 reachability smoke | 新增 `/admin/openwebui` 三語說明頁與 `/api/openwebui/*` narrow bridge；Open WebUI 可匯入專用 tool spec；狀態頁會顯示 token、target user、allowlist、operator-agent mode 風險；新增 `scripts/openwebui_bridge_smoke.py` 可用 bearer token 驗證 tool spec、sessions、stage event 與 chat call；已從 GB10 host 透過 SSH reverse tunnel 驗證 tool spec、stage event 與 chat | `node --check`、`py_compile`、Open WebUI bridge pytest、Open WebUI bridge smoke script、GB10 reverse tunnel smoke | EchoBot 本機只綁 `127.0.0.1`；長期接入仍需要 Cloudflare Tunnel/Tailscale/VPS/可信 reverse proxy 與 Open WebUI 後台 tool server 註冊，不把 token 寫入 repo |
...
| 19 | Stage emotion / motion event | 已完成第一版 | `StageEvent` 與 Open WebUI bridge 可攜帶 `emotion`、`expression`、`motion`；新增 `character_state` event；`/stage` 會依 Live2D config 套用 expression/motion；`/messenger` final 回覆支援 `[emotion:...] [expression:...] [motion:...]` 標籤並只以 `textContent` 顯示文字 | `349 passed, 2 warnings, 16 subtests passed`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描 | 下一步可做角色 package 進階批量管理 |
| 20 | Character emotion map | 已完成本機版 | `/admin/characters` 可為每個角色新增、命名與維護 emotion-to-expression/motion map；`/api/stage/events` 與 Open WebUI bridge 在事件缺少 explicit expression/motion 時，會依目前 session 綁定角色自動套用 map；新增 `scripts/live2d_asset_smoke.py` 驗證 bundled Live2D model3、texture、expression、motion 與 Stage event directive | `349 passed, 2 warnings, 16 subtests passed`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描、Live2D asset smoke | HTTPS 真機前台效果仍需後續驗收 |
| 21 | Character package 匯入/匯出 | 已完成本機版 | `/admin/characters` 可匯出單一角色 JSON package，內容包含 prompt、model profile 綁定、emotion map 與非敏感模型設定快照；匯入可改名或覆蓋既有角色，匯出不包含明文 API key/secrets | `349 passed, 2 warnings, 16 subtests passed`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描 | v1 不打包 Live2D asset，不做批量角色庫 |
...
| Regression | 全量 pytest | 已完成，最新為 `349 passed, 2 warnings, 16 subtests passed` |
...
| 11 | Full regression | Done | This round's full pytest passed: `349 passed, 2 warnings, 16 subtests passed`, including Open WebUI bridge, Model Profiles, Character Profiles, Character Packages, Stage emotion/motion event, channel secret redaction, admin allowlist, channel smoke, gateway user-scope, Telegram/Discord true E2E support, Discord native adapter, and local model roleplay fallback tests | `.venv/bin/python -m pytest -q` | None |
...
| 14 | Open WebUI operator bridge interface | Bridge API, local smoke tooling, and GB10 reachability smoke done | Added `/admin/openwebui` in three languages and a narrow `/api/openwebui/*` bridge; Open WebUI can import the dedicated tool spec; the status page now shows token, target-user, allowlist, and operator-agent mode risk; `scripts/openwebui_bridge_smoke.py` validates the tool spec, sessions, stage events, and chat calls with a bearer token; GB10 host calls have verified tool spec, stage event, and chat through an SSH reverse tunnel | `node --check`, `py_compile`, Open WebUI bridge pytest, Open WebUI bridge smoke script, GB10 reverse tunnel smoke | EchoBot is bound to `127.0.0.1`; long-running integration still needs Cloudflare Tunnel/Tailscale/VPS/trusted reverse proxy reachability plus Open WebUI tool server registration; never commit the bridge token |
...
| Regression | Full pytest | Done, latest `349 passed, 2 warnings, 16 subtests passed` |
