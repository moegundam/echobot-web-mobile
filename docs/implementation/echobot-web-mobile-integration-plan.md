...
| 14 | Open WebUI 操作員 Bridge 介面 | 已完成接口與本機 smoke 工具 | 新增 `/admin/openwebui` 三語說明頁與 `/api/openwebui/*` narrow bridge；Open WebUI 可匯入專用 tool spec；狀態頁會顯示 token、target user、allowlist、operator-agent mode 風險；新增 `scripts/openwebui_bridge_smoke.py` 可用 bearer token 驗證 tool spec、sessions、stage event 與 chat call | `node --check`、`py_compile`、Open WebUI bridge pytest、Open WebUI bridge smoke script、GB10 Open WebUI/LiteLLM endpoint reachability | EchoBot 本機只綁 `127.0.0.1`；GB10 Open WebUI 真正接入仍需要 Tunnel/Tailscale/可信 reverse proxy 與 Open WebUI 後台 tool server 註冊，不把 token 寫入 repo |
...
| 20 | Character emotion map | Local version done | `/admin/characters` can create, name, and maintain emotion-to-expression/motion maps per character; `/api/stage/events` and the Open WebUI bridge apply the current session role's map when events omit explicit expression/motion values; `scripts/live2d_asset_smoke.py` validates bundled Live2D model3, texture, expression, motion, and Stage event directives | `346 passed, 1 warning`, JS syntax, `py_compile`, `git diff --check`, sensitive-string scan, Live2D asset smoke | HTTPS real-device Stage effects still need follow-up validation |
