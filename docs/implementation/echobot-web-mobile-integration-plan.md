...
| 11 | Full regression | 已完成 | 目前working candidate全量pytest：`498 passed, 3 warnings`；外部平台真實E2E另列，不由unit/API suite代稱 | `.venv/bin/python -m pytest` | current-SHA GitHub CI pending |
...
| 17 | Channels 後台通訊平台頁 | 已完成第四版 | `/admin/channels` 支援 Telegram / Discord 設定、secret redaction、admin-gated save、readiness check、Stage target 與 deterministic smoke；Telegram/Discord adapter 與歷史維護者平台 smoke 已存在 | `node --check`、channel targeted pytest、web static pytest、local routing/Stage smoke、歷史外部證據 | 每次部署仍需 fresh external inbound/reply/Session/Stage E2E；LINE/WhatsApp adapter、QQ 長跑仍未完成 |
...
| 19 | Stage emotion / motion event | 已完成第一版 | `StageEvent`與Open WebUI bridge可攜帶emotion/expression/motion；Stage依Live2D config套用；Messenger只以`textContent`顯示 | current `498 passed, 3 warnings` + Stage targeted tests | 真機Stage效果仍需外部驗收 |
| 20 | Character emotion map | 已完成本機版 | `/admin/characters`維護emotion-to-expression/motion map；Stage/OpenWebUI缺explicit值時依Session Character補全 | current `498 passed, 3 warnings` + Live2D asset smoke | HTTPS真機效果pending |
| 21 | Character package 匯入/匯出 | 已完成本機版 | 匯出prompt、LLM/Voice/Live2D binding、emotion map與非敏感快照；匯入可rename/overwrite；不含明文secret | current `498 passed, 3 warnings` + package/redaction tests | v1不含Live2D binary或batch library |
...
| Regression | 全量 pytest | current working candidate：`498 passed, 3 warnings, 31 subtests passed in 28.80s` |
...
| 11 | Full regression | Done | Current working candidate: `498 passed, 3 warnings`; real external platform E2E is tracked separately and is not inferred from the suite | `.venv/bin/python -m pytest` | current-SHA GitHub CI pending |
...
| 17 | Channels admin messaging page | Fourth version done | `/admin/channels` supports Telegram / Discord settings, secret redaction, admin-gated save, readiness checks, Stage targets, and deterministic smoke; adapters and historical maintainer platform smoke exist | `node --check`, targeted channel/static tests, local routing/Stage smoke, historical external evidence | Every deployment still requires fresh external inbound/reply/Session/Stage E2E; LINE/WhatsApp adapters and QQ long-run acceptance remain open |
...
| 19 | Stage emotion / motion event | First version done | Stage/OpenWebUI carry emotion/expression/motion; Messenger renders text through `textContent` | current `498 passed, 3 warnings` plus Stage tests | real-device Stage pending |
| 20 | Character emotion map | Local version done | Admin maintains emotion mappings; Stage/OpenWebUI fill missing directives from the Session Character | current `498 passed, 3 warnings` plus asset smoke | HTTPS real-device pending |
| 21 | Character package import/export | Local version done | exports prompt, bindings, emotion map, and non-secret snapshot; import supports rename/overwrite; no plaintext secrets | current `498 passed, 3 warnings` plus package/redaction tests | v1 excludes Live2D binaries and bulk library |
...
| Regression | Full pytest | Current working candidate: `498 passed, 3 warnings, 31 subtests passed in 28.80s` |
