...
| 11 | Full regression | 已完成 | 本輪全量 pytest 已通過：`346 passed, 1 warning`，含 Open WebUI bridge、Model Profiles、Character Profiles、Character Packages、Stage emotion/motion event、channel secret redaction、admin allowlist、channel smoke、gateway user-scope、Discord native adapter 與 local model roleplay fallback 測試 | `.venv/bin/python -m pytest` | 無 |
...
| 14 | Open WebUI 操作員 Bridge 介面 | 已完成接口與本機 smoke 工具 | 新增 `/admin/openwebui` 三語說明頁與 `/api/openwebui/*` narrow bridge；Open WebUI 可匯入專用 tool spec；狀態頁會顯示 token、target user、allowlist、operator-agent mode 風險；新增 `scripts/openwebui_bridge_smoke.py` 可用 bearer token 驗證 tool spec、sessions 與 stage event | `node --check`、`py_compile`、Open WebUI bridge pytest、Open WebUI bridge smoke script | 真正把遠端 Open WebUI UI 內的 tool server 開啟仍需要該 Open WebUI 的登入/session，不把 token 寫入 repo |
...
| 17 | Channels 後台通訊平台頁 | 已完成第三版 | `/admin/channels` 已支援 Telegram / Discord 設定表單、secret redaction、admin-gated save、`POST /api/channels/{channel}/smoke` readiness check；正式通訊 gateway 可用 `mirror_to_stage` / `stage_session_name` 把回覆同步到 `/stage`；新增 secret-free `GET /api/channels/stage-targets`，讓 `/stage` 與 `/messenger` 可直接選擇 Telegram/Discord 等已設定 target，不必手動輸入 session；Telegram 已用 repo 外 token 驗證 Bot API `getMe`、poller 啟動、Bot API outbound、session 綁定與 Stage target projection，token 只存 ignored runtime config；Discord 現在支援受 secret 保護的 webhook bridge、outbound webhook delivery，以及安裝 `discord.py` 並開啟 Message Content Intent 後的 native bot events | `node --check`、channel targeted pytest、web static pytest、Telegram token/poller/outbound readiness、Stage target projection、Discord webhook bridge tests、Discord native adapter unit tests、敏感字串掃描 | LINE/WhatsApp adapter 與公開 webhook/真實平台長跑測試仍需後續切片；Discord 真伺服器啟用需要填入 repo 外 bot token 與 Discord Developer Portal intent |
| 23 | CI / 公開安全 / Browser smoke 工具 | 已完成本機版 | 新增 GitHub Actions CI，執行 dependency install、JS syntax、Python compile、public safety scan、whitespace check 與 pytest；新增 `scripts/check_public_safety.py` 阻擋 tracked `.echobot/`、非 example env 與常見真 secret；新增 `scripts/browser_smoke.py` 可在安裝 Playwright 後跑多 viewport 頁面 smoke | GitHub Actions workflow review、`python scripts/check_public_safety.py`、`python scripts/browser_smoke.py --help` | CI 尚待推送後由 GitHub Actions 實跑；真瀏覽器 smoke 需要安裝 Playwright/Chromium |
| 18 | Character Profiles 角色設定頁 | 已完成第一版 | 新增 `/admin/characters` 三語角色設定頁與 `/api/character-profiles*` 聚合 API；角色 prompt、model profile 綁定、有效 chat/TTS/ASR/Live2D 摘要可在同一頁管理；資料仍沿用 role card 與 model profile binding，不新增第二套角色儲存格式 | targeted pytest、web static pytest、`node --check`、`py_compile` | 後續補角色 package 進階批量管理 |
| 19 | Stage emotion / motion event | 已完成第一版 | `StageEvent` 與 Open WebUI bridge 可攜帶 `emotion`、`expression`、`motion`；新增 `character_state` event；`/stage` 會依 Live2D config 套用 expression/motion；`/messenger` final 回覆支援 `[emotion:...] [expression:...] [motion:...]` 標籤並只以 `textContent` 顯示文字 | `346 passed, 1 warning`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描 | 下一步可做角色 package 進階批量管理 |
| 20 | Character emotion map | 已完成本機版 | `/admin/characters` 可為每個角色新增、命名與維護 emotion-to-expression/motion map；`/api/stage/events` 與 Open WebUI bridge 在事件缺少 explicit expression/motion 時，會依目前 session 綁定角色自動套用 map | `346 passed, 1 warning`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描 | 真機 Live2D assets 的表情/動作名稱仍需依模型檔驗證 |
| 21 | Character package 匯入/匯出 | 已完成本機版 | `/admin/characters` 可匯出單一角色 JSON package，內容包含 prompt、model profile 綁定、emotion map 與非敏感模型設定快照；匯入可改名或覆蓋既有角色，匯出不包含明文 API key/secrets | `346 passed, 1 warning`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描 | v1 不打包 Live2D asset，不做批量角色庫 |
| 22 | PR 更新與收尾 | 未開始 | PR #1 存在且 mergeable | `git push`、GitHub PR check | 等本機第二輪驗證完成後補 commit |
...
| Open WebUI Bridge | `/admin/openwebui` 說明頁、專用 OpenAPI tool spec、Stage/session/chat bridge API 與本機 bridge smoke script | 已完成接口與本機 smoke 工具；遠端 Open WebUI UI 內啟用 tool server 仍需要該服務的登入/session |
...
| Regression | 全量 pytest | 已完成，`346 passed, 1 warning` |
...
| 11 | Full regression | Done | This round's full pytest passed: `346 passed, 1 warning`, including Open WebUI bridge, Model Profiles, Character Profiles, Character Packages, Stage emotion/motion event, channel secret redaction, admin allowlist, channel smoke, gateway user-scope, Discord native adapter, and local model roleplay fallback tests | `.venv/bin/python -m pytest` | None |
...
| 14 | Open WebUI operator bridge interface | Bridge API and local smoke tooling done | Added `/admin/openwebui` in three languages and a narrow `/api/openwebui/*` bridge; Open WebUI can import the dedicated tool spec; the status page now shows token, target-user, allowlist, and operator-agent mode risk; `scripts/openwebui_bridge_smoke.py` validates the tool spec, sessions, and stage event calls with a bearer token | `node --check`, `py_compile`, Open WebUI bridge pytest, Open WebUI bridge smoke script | Enabling the tool server inside a remote Open WebUI UI still requires that Open WebUI login/session; never commit the bridge token |
...
| 17 | Channels admin messaging page | Third version done | `/admin/channels` now supports Telegram / Discord settings forms, secret redaction, admin-gated save, and `POST /api/channels/{channel}/smoke` readiness checks; production messaging gateways can use `mirror_to_stage` / `stage_session_name` to show replies on `/stage`; added secret-free `GET /api/channels/stage-targets` so `/stage` and `/messenger` can select configured Telegram/Discord targets instead of typing a session; Telegram has passed Bot API `getMe`, poller startup, Bot API outbound, session binding, and Stage target projection checks using a repo-external token stored only in ignored runtime config; Discord now supports a secret-protected webhook bridge, outbound webhook delivery, and native bot events when `discord.py` is installed and Message Content Intent is enabled | `node --check`, channel targeted pytest, web static pytest, Telegram token/poller/outbound readiness, Stage target projection, Discord webhook bridge tests, Discord native adapter unit tests, sensitive-string scan | LINE/WhatsApp adapters and public webhook / long-running real-platform tests remain future slices; Discord real-server enablement needs a repo-external bot token and Discord Developer Portal intent |
| 23 | CI / public safety / browser smoke tooling | Local version done | Added GitHub Actions CI for dependency install, JS syntax, Python compile, public safety scan, whitespace check, and pytest; added `scripts/check_public_safety.py` to block tracked `.echobot/`, non-example env files, and common real-secret patterns; added `scripts/browser_smoke.py` for multi-viewport page smoke checks with Playwright/Chromium | GitHub Actions workflow review, `.venv/bin/python scripts/check_public_safety.py`, `.venv/bin/python scripts/browser_smoke.py --base-url http://127.0.0.1:8001` | GitHub Actions still needs to run after push |
| 18 | Character Profiles setup page | First version done | Added `/admin/characters` in three languages and the composed `/api/character-profiles*` API; role prompts, model profile bindings, and effective chat/TTS/ASR/Live2D summaries are managed in one page; data still reuses role cards and model profile bindings instead of creating a second character store | targeted pytest, web static pytest, `node --check`, `py_compile` | Add advanced bulk character package management later |
| 19 | Stage emotion / motion event | First version done | `StageEvent` and the Open WebUI bridge can carry `emotion`, `expression`, and `motion`; added `character_state` events; `/stage` applies expression/motion from Live2D config; `/messenger` final replies support `[emotion:...] [expression:...] [motion:...]` tags and still render text only through `textContent` | `346 passed, 1 warning`, JS syntax, `py_compile`, `git diff --check`, sensitive-string scan | Next step can be advanced bulk character package management |
| 20 | Character emotion map | Local version done | `/admin/characters` can create, name, and maintain emotion-to-expression/motion maps per character; `/api/stage/events` and the Open WebUI bridge apply the current session role's map when events omit explicit expression/motion values | `346 passed, 1 warning`, JS syntax, `py_compile`, `git diff --check`, sensitive-string scan | Real Live2D asset expression/motion names still need model-file validation |
| 21 | Character package import/export | Local version done | `/admin/characters` can export a single-character JSON package containing the prompt, model profile binding, emotion map, and non-sensitive model settings snapshot; import can rename or overwrite an existing character, and exports never include plaintext API keys/secrets | `346 passed, 1 warning`, JS syntax, `py_compile`, `git diff --check`, sensitive-string scan | v1 does not bundle Live2D assets or provide a bulk character library |
| 22 | PR update and closeout | Not started | PR #1 exists and is mergeable | `git push`, GitHub PR check | Push a follow-up commit after the second local verification round |
...
| Channels | `/admin/channels` shows runtime channel status, Telegram / Discord settings and smoke readiness, while retaining LINE/WhatsApp as follow-up gateway entries | Third version done; Telegram token/poller/outbound readiness, session binding, and Stage target projection have passed; Discord webhook bridge, outbound webhook delivery, and native bot adapter are covered by tests; LINE/WhatsApp runtime adapters and public webhook / long-running real-platform tests remain follow-up work |
...
| Regression | Full pytest | Done, `346 passed, 1 warning` |
