...
| 11 | Full regression | 已完成 | 本輪全量 pytest 已通過：`321 passed`，含 Open WebUI bridge、Model Profiles、Character Profiles、Character Packages、Stage emotion/motion event、channel secret redaction、admin allowlist、channel smoke、gateway user-scope 與 local model roleplay fallback 測試 | `.venv/bin/python -m pytest` | 無 |
...
| 17 | Channels 後台通訊平台頁 | 已完成第二版 | `/admin/channels` 已支援 Telegram / Discord 設定表單、secret redaction、admin-gated save、`POST /api/channels/{channel}/smoke` readiness check；正式通訊 gateway 可用 `mirror_to_stage` / `stage_session_name` 把回覆同步到 `/stage`；新增 secret-free `GET /api/channels/stage-targets`，讓 `/stage` 與 `/messenger` 可直接選擇 Telegram/Discord 等已設定 target，不必手動輸入 session；Telegram 已用 repo 外 token 完成本機 bot polling E2E 與 Stage 同步 smoke，token 只存 ignored runtime config；Discord 已加入 config schema 與 config-only placeholder，LINE/WhatsApp 仍維持規劃入口 | `node --check`、channel targeted pytest、web static pytest、full pytest `321 passed`、Telegram local bot polling E2E、Telegram Stage mirror smoke、敏感字串掃描 | Discord runtime adapter、LINE/WhatsApp adapter 與公開 webhook/真實平台長跑測試仍需後續切片 |
...
| 19 | Stage emotion / motion event | 已完成第一版 | `StageEvent` 與 Open WebUI bridge 可攜帶 `emotion`、`expression`、`motion`；新增 `character_state` event；`/stage` 會依 Live2D config 套用 expression/motion；`/messenger` final 回覆支援 `[emotion:...] [expression:...] [motion:...]` 標籤並只以 `textContent` 顯示文字 | `321 passed`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描 | 下一步可做角色 package 進階批量管理 |
| 20 | Character emotion map | 已完成本機版 | `/admin/characters` 可為每個角色新增、命名與維護 emotion-to-expression/motion map；`/api/stage/events` 與 Open WebUI bridge 在事件缺少 explicit expression/motion 時，會依目前 session 綁定角色自動套用 map | `321 passed`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描 | 真機 Live2D assets 的表情/動作名稱仍需依模型檔驗證 |
| 21 | Character package 匯入/匯出 | 已完成本機版 | `/admin/characters` 可匯出單一角色 JSON package，內容包含 prompt、model profile 綁定、emotion map 與非敏感模型設定快照；匯入可改名或覆蓋既有角色，匯出不包含明文 API key/secrets | `321 passed`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描 | v1 不打包 Live2D asset，不做批量角色庫 |
...
| Channels | `/admin/channels` 顯示 runtime channel 狀態、Telegram / Discord 設定與 smoke readiness，並保留 QQ/LINE/WhatsApp 後續 gateway 入口 | 已完成第二版；Telegram 本機 bot polling E2E 與 Stage 同步已通過；Discord/LINE/WhatsApp runtime adapter 與公開 webhook/真實平台長跑測試仍需後續切片 |
...
| Regression | 全量 pytest | 已完成，`321 passed` |
...
| 11 | Full regression | Done | This round's full pytest passed: `321 passed`, including Open WebUI bridge, Model Profiles, Character Profiles, Character Packages, Stage emotion/motion event, channel secret redaction, admin allowlist, channel smoke, gateway user-scope, and local model roleplay fallback tests | `.venv/bin/python -m pytest` | None |
...
| 17 | Channels admin messaging page | Second version done | `/admin/channels` now supports Telegram / Discord settings forms, secret redaction, admin-gated save, and `POST /api/channels/{channel}/smoke` readiness checks; production messaging gateways can use `mirror_to_stage` / `stage_session_name` to show replies on `/stage`; added secret-free `GET /api/channels/stage-targets` so `/stage` and `/messenger` can select configured Telegram/Discord targets instead of typing a session; Telegram has passed local bot-polling E2E and Stage mirroring smoke using a repo-external token stored only in ignored runtime config; Discord has a config schema plus a config-only placeholder, while LINE/WhatsApp remain planned entries | `node --check`, channel targeted pytest, web static pytest, full pytest `321 passed`, Telegram local bot-polling E2E, Telegram Stage mirror smoke, sensitive-string scan | Discord runtime adapter, LINE/WhatsApp adapters, and public webhook / long-running real-platform tests remain future slices |
...
| Channels | `/admin/channels` shows runtime channel status, Telegram / Discord settings and smoke readiness, while retaining QQ/LINE/WhatsApp as follow-up gateway entries | Second version done; Telegram local bot-polling E2E and Stage mirroring have passed; Discord/LINE/WhatsApp runtime adapters and public webhook / long-running real-platform tests remain follow-up work |
...
| Regression | Full pytest | Done, `321 passed` |
