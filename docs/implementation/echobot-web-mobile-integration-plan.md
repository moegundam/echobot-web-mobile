...
| Channels | `/admin/channels` 顯示 runtime channel 狀態、Telegram / Discord 設定與 smoke readiness，並保留 QQ/LINE/WhatsApp 後續 gateway 入口 | 已完成第二版；Telegram token/poller/outbound readiness、session 綁定與 Stage target projection 已通過；Telegram 使用者 inbound 真實 E2E、Discord/LINE/WhatsApp runtime adapter 與公開 webhook/真實平台長跑測試仍需後續切片 |
