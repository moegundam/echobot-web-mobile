...
| 9 | Cloudflare Tunnel 實際建立與 Access policy | 部分完成 | `cloudflared` 已安裝但尚未完成 Cloudflare browser login / origin cert，尚未建立 named tunnel / DNS route / Access app；本機 app 與 GB10/OpenWebUI reverse tunnel 已改由 macOS launchd 管理，並新增 `scripts/echobot_entrypoint.py` doctor/status/smoke | `scripts/echobot_entrypoint.py doctor/status/smoke-openwebui`、launchd status、HTTPS `/web` 待 Cloudflare | Cloudflare 正式 HTTPS 仍需要登入、hostname、Access email 名單；真機 HTTPS 驗收依使用者指示跳過 |
...
| 11 | Full regression | 已完成 | 本輪全量 pytest 已通過：`352 passed, 2 warnings, 16 subtests passed`，含 Open WebUI bridge、Model Profiles、Character Profiles、Character Packages、Stage emotion/motion event、channel secret redaction、admin allowlist、channel smoke、gateway user-scope、Telegram/Discord true E2E support、Discord native adapter 與 local model roleplay fallback 測試 | `.venv/bin/python -m pytest -q` | 無 |
...
| 14 | Open WebUI 操作員 Bridge 介面 | 已完成接口、本機 smoke、GB10 reachability smoke 與 launchd 穩定入口 | 新增 `/admin/openwebui` 三語說明頁與 `/api/openwebui/*` narrow bridge；Open WebUI 可匯入專用 tool spec；狀態頁會顯示 token、target user、allowlist、operator-agent mode 風險；`scripts/openwebui_bridge_smoke.py` 可用 bearer token 或 runtime-only token file 驗證 tool spec、sessions、stage event 與 chat call；`scripts/echobot_entrypoint.py` 可管理本機 app 與 GB10 reverse tunnel launchd，並從 GB10 主機視角重跑 smoke | `node --check`、`py_compile`、Open WebUI bridge pytest、Open WebUI bridge smoke script、GB10 reverse tunnel smoke、launchd status | Open WebUI 後台 tool server 註冊仍需要該服務 UI 或持久化設定；Cloudflare Tunnel/Access 仍是正式 HTTPS 入口 |
...
| 19 | Stage emotion / motion event | 已完成第一版 | `StageEvent` 與 Open WebUI bridge 可攜帶 `emotion`、`expression`、`motion`；新增 `character_state` event；`/stage` 會依 Live2D config 套用 expression/motion；`/messenger` final 回覆支援 `[emotion:...] [expression:...] [motion:...]` 標籤並只以 `textContent` 顯示文字 | `352 passed, 2 warnings, 16 subtests passed`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描 | 下一步可做角色 package 進階批量管理 |
| 20 | Character emotion map | 已完成本機版 | `/admin/characters` 可為每個角色新增、命名與維護 emotion-to-expression/motion map；`/api/stage/events` 與 Open WebUI bridge 在事件缺少 explicit expression/motion 時，會依目前 session 綁定角色自動套用 map；新增 `scripts/live2d_asset_smoke.py` 驗證 bundled Live2D model3、texture、expression、motion 與 Stage event directive | `352 passed, 2 warnings, 16 subtests passed`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描、Live2D asset smoke | HTTPS 真機前台效果仍需後續驗收 |
| 21 | Character package 匯入/匯出 | 已完成本機版 | `/admin/characters` 可匯出單一角色 JSON package，內容包含 prompt、model profile 綁定、emotion map 與非敏感模型設定快照；匯入可改名或覆蓋既有角色，匯出不包含明文 API key/secrets | `352 passed, 2 warnings, 16 subtests passed`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描 | v1 不打包 Live2D asset，不做批量角色庫 |
...
| Open WebUI Bridge | `/admin/openwebui` 說明頁、專用 OpenAPI tool spec、Stage/session/chat bridge API 與本機 bridge smoke script | 已完成接口、本機 smoke、GB10 reverse smoke 與 launchd 穩定入口；遠端 Open WebUI UI 內啟用 tool server 仍需要該服務的登入/session |
...
| Regression | 全量 pytest | 已完成，最新為 `352 passed, 2 warnings, 16 subtests passed` |
...
| 9 | Create Cloudflare Tunnel and Access policy | Partially done | `cloudflared` is installed but Cloudflare browser login / origin cert is not complete; no named tunnel / DNS route / Access app yet. The local app and GB10/OpenWebUI reverse tunnel now run under macOS launchd, with `scripts/echobot_entrypoint.py` doctor/status/smoke support | `scripts/echobot_entrypoint.py doctor/status/smoke-openwebui`, launchd status; HTTPS `/web` pending Cloudflare | Formal Cloudflare HTTPS still needs login, hostname, and Access email list; real-device HTTPS validation is skipped per user instruction |
...
| 11 | Full regression | Done | This round's full pytest passed: `352 passed, 2 warnings, 16 subtests passed`, including Open WebUI bridge, Model Profiles, Character Profiles, Character Packages, Stage emotion/motion event, channel secret redaction, admin allowlist, channel smoke, gateway user-scope, Telegram/Discord true E2E support, Discord native adapter, and local model roleplay fallback tests | `.venv/bin/python -m pytest -q` | None |
...
| Regression | Full pytest | Done, latest `352 passed, 2 warnings, 16 subtests passed` |
