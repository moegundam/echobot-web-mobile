...
| 19 | Stage emotion / motion event | 已完成第一版 | `StageEvent` 與 Open WebUI bridge 可攜帶 `emotion`、`expression`、`motion`；新增 `character_state` event；`/stage` 會依 Live2D config 套用 expression/motion；`/messenger` final 回覆支援 `[emotion:...] [expression:...] [motion:...]` 標籤並只以 `textContent` 顯示文字 | `312 passed`、JS syntax、`py_compile`、`git diff --check`、敏感字串掃描 | 後續可在角色頁補 emotion-to-expression map 與 UI 選單 |
| 20 | PR 更新與收尾 | 未開始 | PR #1 存在且 mergeable | `git push`、GitHub PR check | 等本機第二輪驗證完成後補 commit |
...
| Stage Emotion | Stage event 與 Open WebUI bridge 可推送 emotion/expression/motion，`/stage` 可播放對應 Live2D 表情/動作 | 已完成第一版；真機 Live2D 模型動作效果仍需視模型 assets 驗證 |
...
| Stage Emotion | Stage events and Open WebUI bridge can push emotion/expression/motion, and `/stage` can play matching Live2D expressions/motions | First version done; real-device/model-asset motion effect still needs validation |
