...
| `/admin/characters` | 角色設定 | 角色 prompt、模型 profile 綁定、TTS/ASR/Live2D 摘要、emotion-to-expression/motion map |
...
| `/api/character-profiles*` | `/admin/characters` | 角色 prompt、模型 profile 綁定與 emotion map 的聚合 API |
...
| `/admin/characters` | Character setup | Role prompt, model profile binding, TTS/ASR/Live2D summary, and emotion-to-expression/motion maps |
...
| `/api/character-profiles*` | `/admin/characters` | Composed API for role prompts, model profile bindings, and emotion maps |
