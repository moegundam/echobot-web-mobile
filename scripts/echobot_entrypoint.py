...
def _build_app_plist(config: dict[str, object]) -> dict[str, object]:
    ...
    command = " ".join(
        [
            ...
            "&&",
            'if [ -n "${ECHOBOT_OPENWEBUI_BRIDGE_TOKEN_FILE:-}" ] && [ -s "$ECHOBOT_OPENWEBUI_BRIDGE_TOKEN_FILE" ]; then',
            'export ECHOBOT_OPENWEBUI_BRIDGE_TOKEN="$(cat "$ECHOBOT_OPENWEBUI_BRIDGE_TOKEN_FILE")";',
            "fi;",
            "exec",
