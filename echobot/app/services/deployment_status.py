...
from .openwebui_bridge import openwebui_bridge_status
...
class DeploymentStatusService:
    """Build the deployment status payload without blocking the event loop."""

    def __init__(self, runtime: Any) -> None:
        self._runtime = runtime

    async def snapshot(self) -> dict[str, Any]:
        health = await self._runtime.health_snapshot()
        system = await asyncio.to_thread(
            _system_snapshot,
            self._runtime.workspace,
        )
        return _deployment_status_payload(
            health=health,
...
def _system_snapshot(workspace: Path) -> dict[str, Any]:
    return {
        "cloudflare": _cloudflared_status(),
        "github_actions": _github_actions_status(workspace),
        "source_identity": _source_identity_snapshot(workspace),
...
def _deployment_status_payload(
    *,
    ...
    return {
        ...
        "github_actions": ci,
        "source_identity": system["source_identity"],
        "openwebui": system["openwebui"],
        "simple_deploy": {
        ...
        "commands": [
            ...
            f"python scripts/openwebui_bridge_smoke.py --base-url {public_base_url} --session-name demo",
...
def _public_base_url() -> str:
    value = os.environ.get("ECHOBOT_PUBLIC_BASE_URL", "").strip().rstrip("/")
    if value and re.fullmatch(r"https?://[^\s]+", value):
        return value
    return "<ECHOBOT-BASE-URL>"


def _cloudflared_status() -> dict[str, Any]:
    binary = shutil.which("cloudflared")
    version = _command_output([binary, "--version"]) if binary else ""
...
def _source_identity_snapshot(workspace: Path) -> dict[str, object]:
    version = _git_output(["rev-parse", "--short", "HEAD"], workspace)
    branch = _git_output(["rev-parse", "--abbrev-ref", "HEAD"], workspace)
    status = _git_output(
...
def _git_output(command: list[str], workspace: Path) -> str | None:
    try:
